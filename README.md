...
**Separation of concerns** — resolvers are intentionally thin. They receive arguments and delegate to services. All Open Meteo HTTP calls live in `openMeteo.ts` and all scoring logic lives in `activityScorer.ts`. This makes each piece independently testable.
